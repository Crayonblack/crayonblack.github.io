import { r as registerInstance, h, g as getElement } from './index-b2529a41.js';
...
var _config$1 = {
  ...
  return _isFunction$3(value) || _isString$4(value);
},
...
_unitExp = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i,
  ...
  var target = targets[0],
      harnessPlugin,
      i;
  _isObject$1(target) || _isFunction$3(target) || (targets = [targets]);
...
clamp = function clamp(min, max, value) {
  ...
  return value && _isObject$1(value) && "length" in value && (!nonEmpty && !value.length || value.length - 1 in value && _isObject$1(value[0])) && !value.nodeType && value !== _win$6;
},
...
distribute = function distribute(v) {
  ...
  if (_windowExists$3() || config.headless) {
    ...
    config.register && config.register(gsap$9, Plugin, PropTween$1);
  } else {
...
_parseKeyframe = function _parseKeyframe(prop, obj, allProps, easeEach) {
  ...
  return _isFunction$3(value) ? value.call(tween, i, target, targets) : _isString$4(value) && ~value.indexOf("random(") ? _replaceRandom(value) : value;
},
...
var _media = [],
  ...
  if (time - _lastMediaTime > 2) {
    ...
    _media.forEach(function (c) {
      ...
      for (p in queries) {
        match = _win$6.matchMedia(queries[p]).matches; // Firefox doesn't update the "matches" property of the MediaQueryList object correctly - it only does so as it calls its change handler - so we must re-create a media query here to ensure it's accurate.
...
var Context = /*#__PURE__*/function () {
  ...
  _proto5.add = function add(name, func, scope) {
    ...
    self.last = f;
    return name === _isFunction$3 ? f(self, function (func) {
...
var MatchMedia = /*#__PURE__*/function () {
  ...
  _proto6.add = function add(conditions, func, scope) {
    ...
    var context = new Context(0, scope || this.scope),
        ...
        p,
        active;
    _context$5 && !context.selector && (context.selector = _context$5.selector); // in case a context is created inside a context. Like a gsap.matchMedia() that's inside a scoped gsap.context()
    ...
    for (p in conditions) {
      if (p === "all") {
        ...
        mq = _win$6.matchMedia(conditions[p]);
...
var _gsap = {
  ...
  quickTo: function quickTo(target, property, vars) {
    ...
    var tween = gsap$9.to(target, _setDefaults$1((_setDefaults2 = {}, _setDefaults2[property] = "+=0.1", _setDefaults2.paused = true, _setDefaults2.stagger = 0, _setDefaults2), vars || {})),
...
_renderRoundedCSSProp = function _renderRoundedCSSProp(ratio, data) {
  ...
  target._gsap || gsap$9.core.getCache(target); // just make sure there's a _gsap cache defined because we read from it in _saveStyle() and it's more efficient to just check it here once.
  ...
  if (_windowExists$2() && window.document) {
    ...
    _reverting$1 = gsap$9.core.reverting;
    _pluginInitted = 1;
  }
},
  ...
  var owner = target.ownerSVGElement,
      svg = _createElement$2("svg", owner && owner.getAttribute("xmlns") || "http://www.w3.org/2000/svg"),
...
_identity2DMatrix = [1, 0, 0, 1, 0, 0],
  ...
  return _isNullTransform(matrixString) ? _identity2DMatrix : matrixString.substr(7).match(_numExp$2).map(_round$3);
},
...
var CSSPlugin = {
  ...
  init: function init(target, vars, tween, index, targets) {
    var props = this._props,
        ...
        hasPriority,
        inlineProps;
    _pluginInitted || _initCore$6(); // we may call init() multiple times on the same plugin instance, like when adding special properties, so make sure we don't overwrite the revert data or inlineProps
...
gsap$9.core.getStyleSaver = _getStyleSaver$1;
...
function getRawPath(value) {
  value = _isString$3(value) && _selectorExp$2.test(value) ? document.querySelector(value) || value : value;
...
function getRotationAtBezierT(segment, i, t) {
  ...
  b += (c - b) * t;
  a += (b - a) * t;
  return _round$2(_atan2$1(b + (c + (segment[i + 7] - c) * t - b) * t - a, x) * _RAD2DEG);
}
...
function subdivideSegment(segment, i, t) {
  ...
  x2 += (x2a - x2) * t;
  y2 += (y2a - y2) * t;
  segment.splice(i + 2, 4, _round$2(x1a), //first control point
  _round$2(y1a), _round$2(x1), //second control point
  _round$2(y1), _round$2(x1 + (x2 - x1) * t), //new fabricated anchor on line
  _round$2(y1 + (y2 - y1) * t), _round$2(x2), //third control point
  _round$2(y2), _round$2(x2a), //fourth control point
  _round$2(y2a));
...
function bezierToPoints(x1, y1, x2, y2, x3, y3, x4, y4, threshold, points, index) {
  var x12 = (x1 + x2) / 2,
      ...
      dx = x4 - x1,
      dy = y4 - y1,
      d2 = _abs$1((x2 - x4) * dy - (y2 - y4) * dx),
      d3 = _abs$1((x3 - x4) * dy - (y3 - y4) * dx),
      length;
...
function pointsToSegment(points, curviness) {
  ...
  for (i = 2; i < l; i += 2) {
    ...
    dx2 = nextX - x;
    dy2 = nextY - y;
    r1 = _sqrt$1(dx1 * dx1 + dy1 * dy1); // r1, r2, and r3 correlate x and y (and z in the future). Basically 2D or 3D hypotenuse
    ...
    if (x !== prevX || y !== prevY) {
      segment.push(_round$2(mx1 + mxm), // first control point
      _round$2(my1 + mym), _round$2(x), // anchor
      _round$2(y), _round$2(mx2 + mxm), // second control point
      _round$2(my2 + mym));
    }
  }
...
var _numbersExp = /(?:(-)?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/ig,
  ...
  return _lastInteraction = _getTime$2();
},
    _CTRL,
    _ALT,
    _SHIFT,
...
_comma = ",",
  ...
  if (!e.target._gsSelection && !_isPressed && _getTime$2() - _lastInteraction > 100) {
  ...
  _removeListener$2(e.target, "touchend", _onMultiTouchDocumentEnd);
},
    _onMultiTouchDocument = function _onMultiTouchDocument(e) {
  _isMultiTouching = e.touches && _dragCount < e.touches.length;

  _addListener$2(e.target, "touchend", _onMultiTouchDocumentEnd);
},
  ...
  return function (e) {
    ...
    _callback$1 = function _callback(type, self, param) {
  ...
  _copyElement$1.style.display = "none";
  _body$4 && _body$4.appendChild(_copyElement$1);
  ...
  _body$4 && _body$4.addEventListener("touchstart", _emptyFunc$1); //works around Safari bug: https://gsap.com/forums/topic/21450-draggable-in-iframe-on-mobile-is-buggy/
  ...
  if (_touchEventLookup[e.type]) {
    ...
    _addListener$2(touchEventTarget, "touchmove", self._onMove);
  ...
  if (touchEventTarget) {
    _removeListener$2(touchEventTarget, "touchend", self._onRelease);

    _removeListener$2(touchEventTarget, "touchmove", self._onMove);
...
var DraggableSVG = /*#__PURE__*/function () {
  ...
  _proto.setPointerPosition = function setPointerPosition(pointerX, pointerY) {
    ...
    if (this.x !== x || this.y !== y) {
      ...
      if (!this.isDragging && this.isPressed) {
        ...
        _callback$1("onDragStart", this, this.pointerEvent);
      }
    }
  };
  ...
  _proto.enabled = function enabled(_enabled) {
    ...
    if (_enabled) {
      if (!_supportsPointer) {
        _addListener$2(this.target, "mousedown", this._onPress);
      }

      _addListener$2(this.target, "touchstart", this._onPress);

      _addListener$2(this.target, "click", this._onClick, true); //note: used to pass true for capture but it prevented click-to-play-video functionality in Firefox.

    } else {
      dragging = this.isDragging;

      _removeListener$2(this.target, "mousedown", this._onPress);

      _removeListener$2(this.target, "touchstart", this._onPress);

      _removeListener$2(_win$3, "touchforcechange", _preventDefault);

      _removeListener$2(this.target, "click", this._onClick);

      if (this.touchEventTarget) {
        _removeListener$2(this.touchEventTarget, "touchcancel", this._onRelease);

        _removeListener$2(this.touchEventTarget, "touchend", this._onRelease);
      ...
      if (dragging) {
        _callback$1("onDragEnd", this, this.pointerEvent);
      }
    }
...
var Anchor = /*#__PURE__*/function () {
  ...
  _proto2.onPress = function onPress() {
    _callback$1("onPress", this);
  };

  _proto2.onClick = function onClick() {
    _callback$1("onClick", this);
  };
  ...
  _proto2.onDragEnd = function onDragEnd() {
    _callback$1("onDragEnd", this);
  };

  _proto2.onRelease = function onRelease() {
    _callback$1("onRelease", this);
  };
...
var PathEditor = /*#__PURE__*/function () {
  function PathEditor(target, vars) {
    ...
    if (!_supportsPointer) {
      _addListener$2(this._selectionHittest, "mousedown", _bind(this._onClickSelectionPath, this));

      _addListener$2(this._selectionHittest, "mouseup", _bind(this._onRelease, this));
    }

    _addListener$2(this._selectionHittest, "touchstart", _bind(this._onClickSelectionPath, this));

    _addListener$2(this._selectionHittest, "touchend", _bind(this._onRelease, this));
  ...
  _proto3._onRelease = function _onRelease(e) {
    ...
    _removeListener$2(_win$3, "touchforcechange", _preventDefault); //otherwise iOS will scroll when dragging.


    _callback$1("onRelease", this, e);
  };
...
PathEditor.register = _initCore$5;
...
_selectorExp$1 = /(^[#\.][a-z]|[a-y][a-z])/i,
  ...
  var bounds = target.getBoundingClientRect(),
      windowOffsetY = _docEl$3.clientTop - (_win$2.pageYOffset || _docEl$3.scrollTop || _body$3.scrollTop || 0),
      windowOffsetX = _docEl$3.clientLeft - (_win$2.pageXOffset || _docEl$3.scrollLeft || _body$3.scrollLeft || 0);
  return {
  ...
  var tweens = gsap$7.getTweensOf(target),
      i = 0;
  ...
  for (; i < tweens.length; i++) {
    ...
    _initCore$4 = function _initCore(core, required) {
...
var MotionPathHelper = /*#__PURE__*/function () {
  function MotionPathHelper(targetOrTween, vars) {
    ...
    if (targetOrTween instanceof gsap$7.core.Tween) {
      animation = targetOrTween;
      target = animation.targets()[0];
    } else {
      target = gsap$7.utils.toArray(targetOrTween)[0];
    ...
    startX = parseFloat(gsap$7.getProperty(target, "x", "px"));
    startY = parseFloat(gsap$7.getProperty(target, "y", "px"));
    ...
    (gsap$7.utils.toArray(vars.container)[0] || _body$3).appendChild(copyButton);

    _addCopyToClipboard(copyButton, function () {
      return self.getString();
    }, function () {
      return gsap$7.fromTo(copyButton, {
    ...
    if (!svg) {
      ...
      temp = _isString$2(path) && !_selectorExp$1.test(path) ? path : _getInitialPath(startX, startY);
      ...
      vars.pathColor && gsap$7.set(path, {
        stroke: vars.pathColor
      });
      vars.pathWidth && gsap$7.set(path, {
        strokeWidth: vars.pathWidth
      });
      vars.pathOpacity && gsap$7.set(path, {
    ...
    if (animation) {
      ...
      if (animationToScrub.parent !== gsap$7.globalTimeline) {
        gsap$7.globalTimeline.add(animationToScrub, _getGlobalTime(animationToScrub) - animationToScrub.delay());
      }

      animationToScrub.repeat(-1).repeatDelay(1);
    } else {
      animation = animationToScrub = gsap$7.to(target, {
        motionPath: {
          path: path,
          start: vars.start || 0,
...
MotionPathHelper.register = _initCore$4;
...
_segmentToRawPath = function _segmentToRawPath(plugin, segment, target, x, y, slicer, vars, unitX, unitY) {
  ...
  if (align) {
    if (align === "self" || (alignTarget = _toArray$4(align)[0] || target) === target) {
...
_getGSAP$5() && gsap$6.registerPlugin(MotionPathPlugin);
...
_getGSAP$4() && gsap$5.registerPlugin(MorphSVGPlugin);
...
_spacesExp = /\s+/g,
  ...
  _coreInitted$4 = gsap$4 = _getGSAP$3();
};
...
_getGSAP$3() && gsap$4.registerPlugin(ScrambleTextPlugin);
...
var gsap$1,
  ...
  if (_windowExists() && gsap$1 && typeof document !== "undefined" && document.body) {
    _window = window;
    _body = document.body;
...
ScrollToPlugin.config = function (vars) {
  _config || _initCore() || (_config = gsap$1.config()); // in case the window hasn't been defined yet.
...
const appRootCss = "@layer base{*,::backdrop,:after,:before{--tw-translate-x:0;--tw-translate-y:0;--tw-translate-z:0;--tw-border-style:solid;--tw-gradient-from:#0000;--tw-gradient-via:#0000;--tw-gradient-to:#0000;--tw-gradient-from-position:0%;--tw-gradient-via-position:50%;--tw-gradient-to-position:100%;--tw-shadow:0 0 #0000;--tw-shadow-alpha:100%;--tw-inset-shadow:0 0 #0000;--tw-inset-shadow-alpha:100%;--tw-ring-shadow:0 0 #0000;--tw-inset-ring-shadow:0 0 #0000;--tw-ring-offset-width:0;--tw-ring-offset-color:#fff;--tw-ring-offset-shadow:0 0 #0000;--tw-scale-x:1;--tw-scale-y:1;--tw-scale-z:1;border:0 solid;box-sizing:border-box;margin:0;padding:0}}/*! tailwindcss v4.1.13 | MIT License | https://tailwindcss.com */@layer properties{@supports (((-webkit-hyphens:none)) and (not (margin-trim:inline))) or ((-moz-orient:inline) and (not (color:rgb(from red r g b)))){*,::backdrop,:after,:before{--tw-translate-x:0;--tw-translate-y:0;--tw-translate-z:0;--tw-border-style:solid;--tw-gradient-position:initial;--tw-gradient-from:#0000;--tw-gradient-via:#0000;--tw-gradient-to:#0000;--tw-gradient-stops:initial;--tw-gradient-via-stops:initial;--tw-gradient-from-position:0%;--tw-gradient-via-position:50%;--tw-gradient-to-position:100%;--tw-shadow:0 0 #0000;--tw-shadow-color:initial;--tw-shadow-alpha:100%;--tw-inset-shadow:0 0 #0000;--tw-inset-shadow-color:initial;--tw-inset-shadow-alpha:100%;--tw-ring-color:initial;--tw-ring-shadow:0 0 #0000;--tw-inset-ring-color:initial;--tw-inset-ring-shadow:0 0 #0000;--tw-ring-inset:initial;--tw-ring-offset-width:0px;--tw-ring-offset-color:#fff;--tw-ring-offset-shadow:0 0 #0000}}}@layer theme{:host,:root{--font-mono:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,\"Liberation Mono\",\"Courier New\",monospace;--color-indigo-50:oklch(96.2% .018 272.314);--color-indigo-700:oklch(45.7% .24 277.023);--spacing:.25rem;--text-lg:1.125rem;--text-lg--line-height:1.55556;--radius-sm:.25rem;--radius-lg:.5rem;--default-font-family:\"Montserrat\",var(--default-font);--default-mono-font-family:var(--font-mono);--default-font:ui-sans-serif,system-ui,sans-serif,\"Apple Color Emoji\",\"Segoe UI Emoji\",\"Segoe UI Symbol\",\"Noto Color Emoji\"}}@layer base{::file-selector-button{appearance:button;background-color:#0000;border:0 solid;border-radius:0;box-sizing:border-box;color:inherit;font:inherit;font-feature-settings:inherit;font-variation-settings:inherit;letter-spacing:inherit;margin:0;margin-inline-end:4px;opacity:1;padding:0}:host,html{-webkit-text-size-adjust:100%;font-family:var(--default-font-family,ui-sans-serif,system-ui,sans-serif,\"Apple Color Emoji\",\"Segoe UI Emoji\",\"Segoe UI Symbol\",\"Noto Color Emoji\");font-feature-settings:var(--default-font-feature-settings,normal);font-variation-settings:var(--default-font-variation-settings,normal);tab-size:4;-webkit-tap-highlight-color:transparent;line-height:1.5}hr{border-top-width:1px;color:inherit;height:0}abbr:where([title]){-webkit-text-decoration:underline dotted;text-decoration:underline dotted}h1,h2,h3,h4,h5,h6{font-size:inherit;font-weight:inherit}a{color:inherit;-webkit-text-decoration:inherit;text-decoration:inherit}b,strong{font-weight:bolder}code,kbd,pre,samp{font-family:var(--default-mono-font-family,ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,\"Liberation Mono\",\"Courier New\",monospace);font-feature-settings:var(--default-mono-font-feature-settings,normal);font-size:1em;font-variation-settings:var(--default-mono-font-variation-settings,normal)}small{font-size:80%}sub,sup{font-size:75%;line-height:0;position:relative;vertical-align:baseline}sub{bottom:-.25em}sup{top:-.5em}table{border-collapse:collapse;border-color:inherit;text-indent:0}:-moz-focusring{outline:auto}progress{vertical-align:baseline}summary{display:list-item}menu,ol,ul{list-style:none}audio,canvas,embed,iframe,img,object,svg,video{display:block;vertical-align:middle}img,video{height:auto;max-width:100%}button,input,optgroup,select,textarea{background-color:#0000;border-radius:0;color:inherit;font:inherit;font-feature-settings:inherit;font-variation-settings:inherit;letter-spacing:inherit;opacity:1}:where(select:is([multiple],[size])) optgroup{font-weight:bolder}:where(select:is([multiple],[size])) optgroup option{padding-inline-start:20px}::placeholder{opacity:1}@supports (not ((-webkit-appearance:-apple-pay-button))) or (contain-intrinsic-size:1px){::placeholder{color:currentColor}@supports (color:color-mix(in lab,red,red)){::placeholder{color:currentColor;color:color-mix(in oklab,currentcolor 50%,transparent)}}}textarea{resize:vertical}::-webkit-search-decoration{-webkit-appearance:none}::-webkit-date-and-time-value{min-height:1lh;min-height:1.5em;text-align:inherit}::-webkit-datetime-edit{display:inline-flex}::-webkit-datetime-edit,::-webkit-datetime-edit-year-field{padding-block:0;padding-bottom:0;padding-top:0}::-webkit-datetime-edit-day-field,::-webkit-datetime-edit-month-field{padding-block:0;padding-bottom:0;padding-top:0}::-webkit-datetime-edit-hour-field,::-webkit-datetime-edit-minute-field{padding-block:0;padding-bottom:0;padding-top:0}::-webkit-datetime-edit-millisecond-field,::-webkit-datetime-edit-second-field{padding-block:0;padding-bottom:0;padding-top:0}::-webkit-datetime-edit-meridiem-field{padding-block:0;padding-bottom:0;padding-top:0}::-webkit-calendar-picker-indicator{line-height:1}:-moz-ui-invalid{box-shadow:none}button,input:where([type=button],[type=reset],[type=submit]){appearance:button}::-webkit-inner-spin-button,::-webkit-outer-spin-button{height:auto}[hidden]:where(:not([hidden=until-found])){display:none!important}[multiple],[type=date],[type=datetime-local],[type=email],[type=month],[type=number],[type=password],[type=search],[type=tel],[type=text],[type=time],[type=url],[type=week],input:where(:not([type])),select,textarea{appearance:none;--tw-shadow:0 0 #0000;background-color:#fff;border-color:oklch(55.1% .027 264.364);border-radius:0;border-width:1px;font-size:1rem;line-height:1.5rem;padding:.5rem .75rem}:is([type=text],input:where(:not([type])),[type=email],[type=url],[type=password],[type=number],[type=date],[type=datetime-local],[type=month],[type=search],[type=tel],[type=time],[type=week],[multiple],textarea,select):focus{outline-offset:2px;--tw-ring-inset:var(--tw-empty, );--tw-ring-offset-width:0px;--tw-ring-offset-color:#fff;--tw-ring-color:oklch(54.6% .245 262.881);--tw-ring-offset-shadow:var(--tw-ring-inset)0 0 0 var(--tw-ring-offset-width)var(--tw-ring-offset-color);--tw-ring-shadow:var(--tw-ring-inset)0 0 0 calc(1px + var(--tw-ring-offset-width))var(--tw-ring-color);border-color:oklch(54.6% .245 262.881);box-shadow:var(--tw-ring-offset-shadow),var(--tw-ring-shadow),var(--tw-shadow);outline:2px solid #0000}input::placeholder,textarea::placeholder{color:oklch(55.1% .027 264.364);opacity:1}::-webkit-datetime-edit-fields-wrapper{padding:0}select{background-image:url(\"data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3E%3Cpath stroke='oklch(55.1%25 0.027 264.364)' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='m6 8 4 4 4-4'/%3E%3C/svg%3E\");background-position:right .5rem center;background-repeat:no-repeat;background-size:1.5em 1.5em;padding-right:2.5rem;print-color-adjust:exact}[multiple],[size]:where(select:not([size=\"1\"])){background-image:none;background-position:0 0;background-repeat:unset;background-size:initial;padding-right:.75rem;print-color-adjust:unset}[type=checkbox],[type=radio]{appearance:none;color:oklch(54.6% .245 262.881);print-color-adjust:exact;-webkit-user-select:none;user-select:none;vertical-align:middle;--tw-shadow:0 0 #0000;background-color:#fff;background-origin:border-box;border-color:oklch(55.1% .027 264.364);border-width:1px;display:inline-block;flex-shrink:0;height:1rem;padding:0;width:1rem}[type=checkbox]{border-radius:0}[type=radio]{border-radius:100%}[type=checkbox]:focus,[type=radio]:focus{outline-offset:2px;--tw-ring-inset:var(--tw-empty, );--tw-ring-offset-width:2px;--tw-ring-offset-color:#fff;--tw-ring-color:oklch(54.6% .245 262.881);--tw-ring-offset-shadow:var(--tw-ring-inset)0 0 0 var(--tw-ring-offset-width)var(--tw-ring-offset-color);--tw-ring-shadow:var(--tw-ring-inset)0 0 0 calc(2px + var(--tw-ring-offset-width))var(--tw-ring-color);box-shadow:var(--tw-ring-offset-shadow),var(--tw-ring-shadow),var(--tw-shadow);outline:2px solid #0000}[type=checkbox]:checked,[type=radio]:checked{background-color:currentColor;background-position:50%;background-repeat:no-repeat;background-size:100% 100%;border-color:#0000}[type=checkbox]:checked{background-image:url(\"data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' fill='%23fff' viewBox='0 0 16 16'%3E%3Cpath d='M12.207 4.793a1 1 0 0 1 0 1.414l-5 5a1 1 0 0 1-1.414 0l-2-2a1 1 0 0 1 1.414-1.414L6.5 9.086l4.293-4.293a1 1 0 0 1 1.414 0'/%3E%3C/svg%3E\")}[type=radio]:checked{background-image:url(\"data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' fill='%23fff' viewBox='0 0 16 16'%3E%3Ccircle cx='8' cy='8' r='3'/%3E%3C/svg%3E\")}[type=checkbox]:checked:focus,[type=checkbox]:checked:hover,[type=radio]:checked:focus,[type=radio]:checked:hover{background-color:currentColor;border-color:#0000}[type=checkbox]:indeterminate{background-color:currentColor;background-image:url(\"data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 16 16'%3E%3Cpath stroke='%23fff' stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M4 8h8'/%3E%3C/svg%3E\");background-position:50%;background-repeat:no-repeat;background-size:100% 100%;border-color:#0000}[type=checkbox]:indeterminate:focus,[type=checkbox]:indeterminate:hover{background-color:currentColor;border-color:#0000}[type=file]{background:unset;border-color:inherit;border-radius:0;border-width:0;font-size:unset;line-height:inherit;padding:0}[type=file]:focus{outline:1px solid buttontext;outline:1px auto -webkit-focus-ring-color}}@layer components;@layer utilities{.absolute{position:absolute}.relative{position:relative}.-left-\\[278px\\]{left:-278px}.z-0{z-index:0}.z-10{z-index:10}.mx-auto{margin-inline:auto}.mt-4{margin-top:calc(var(--spacing)*4)}.mb-3{margin-bottom:calc(var(--spacing)*3)}.flex{display:flex}.h-\\[38\\%\\]{height:38%}.h-\\[60\\%\\]{height:60%}.h-fit{height:fit-content}.h-full{height:100%}.h-screen{height:100vh}.min-h-screen{min-height:100vh}.w-fit{width:fit-content}.w-full{width:100%}.w-screen{width:100vw}.-translate-\\[120\\%\\]{--tw-translate-x:-120%;--tw-translate-y:-120%}.-translate-\\[120\\%\\],.-translate-y-28{translate:var(--tw-translate-x)var(--tw-translate-y)}.-translate-y-28{--tw-translate-y:calc(var(--spacing)*-28)}.translate-y-100{--tw-translate-y:calc(var(--spacing)*100);translate:var(--tw-translate-x)var(--tw-translate-y)}.flex-col{flex-direction:column}.flex-wrap{flex-wrap:wrap}.items-center{align-items:center}.justify-between{justify-content:space-between}.justify-end{justify-content:flex-end}.gap-1{gap:calc(var(--spacing)*1)}.gap-2{gap:calc(var(--spacing)*2)}.gap-4{gap:calc(var(--spacing)*4)}.gap-6{gap:calc(var(--spacing)*6)}.gap-\\[2\\%\\]{gap:2%}.border{border-style:var(--tw-border-style);border-width:1px}.bg-gradient-to-r{--tw-gradient-position:to right in oklab;background-image:linear-gradient(var(--tw-gradient-stops))}.from-indigo-700{--tw-gradient-from:var(--color-indigo-700);--tw-gradient-stops:var(--tw-gradient-via-stops,var(--tw-gradient-position),var(--tw-gradient-from)var(--tw-gradient-from-position),var(--tw-gradient-to)var(--tw-gradient-to-position))}.via-transparent{--tw-gradient-via:transparent;--tw-gradient-via-stops:var(--tw-gradient-position),var(--tw-gradient-from)var(--tw-gradient-from-position),var(--tw-gradient-via)var(--tw-gradient-via-position),var(--tw-gradient-to)var(--tw-gradient-to-position);--tw-gradient-stops:var(--tw-gradient-via-stops)}.via-40\\%{--tw-gradient-via-position:40%}.to-transparent{--tw-gradient-to:transparent;--tw-gradient-stops:var(--tw-gradient-via-stops,var(--tw-gradient-position),var(--tw-gradient-from)var(--tw-gradient-from-position),var(--tw-gradient-to)var(--tw-gradient-to-position))}.px-5{padding-inline:calc(var(--spacing)*5)}.pt-\\[70px\\]{padding-top:70px}.pr-3{padding-right:calc(var(--spacing)*3)}.font-sans{font-family:\"Montserrat\",var(--default-font)}.text-lg{font-size:var(--text-lg);line-height:var(--tw-leading,var(--text-lg--line-height))}.text-indigo-700{color:var(--color-indigo-700)}.opacity-0{opacity:0}.shadow{--tw-shadow:0 1px 3px 0 var(--tw-shadow-color,#0000001a),0 1px 2px -1px var(--tw-shadow-color,#0000001a);box-shadow:var(--tw-inset-shadow),var(--tw-inset-ring-shadow),var(--tw-ring-offset-shadow),var(--tw-ring-shadow),var(--tw-shadow)}}/*! tailwindcss v4.1.13 | MIT License | https://tailwindcss.com */@layer properties{@supports (((-webkit-hyphens:none)) and (not (margin-trim:inline))) or ((-moz-orient:inline) and (not (color:rgb(from red r g b)))){*,::backdrop,:after,:before{--tw-translate-x:0;--tw-translate-y:0;--tw-translate-z:0;--tw-border-style:solid;--tw-gradient-position:initial;--tw-gradient-from:#0000;--tw-gradient-via:#0000;--tw-gradient-to:#0000;--tw-gradient-stops:initial;--tw-gradient-via-stops:initial;--tw-gradient-from-position:0%;--tw-gradient-via-position:50%;--tw-gradient-to-position:100%;--tw-shadow:0 0 #0000;--tw-shadow-color:initial;--tw-shadow-alpha:100%;--tw-inset-shadow:0 0 #0000;--tw-inset-shadow-color:initial;--tw-inset-shadow-alpha:100%;--tw-ring-color:initial;--tw-ring-shadow:0 0 #0000;--tw-inset-ring-color:initial;--tw-inset-ring-shadow:0 0 #0000;--tw-ring-inset:initial;--tw-ring-offset-width:0px;--tw-ring-offset-color:#fff;--tw-ring-offset-shadow:0 0 #0000;--tw-scale-x:1;--tw-scale-y:1;--tw-scale-z:1}}}@layer base{}@layer components;@layer utilities{.-translate-\\[120\\%\\],.-translate-y-28{translate:var(--tw-translate-x)var(--tw-translate-y)}}body *{font-family:\"Montserrat\",var(--default-font)}.wrapper{height:100vh}h1,h2,h3,h4,h5,h6{font-family:\"Dongle\",var(--default-font)}h1{font-size:48px}h2{font-size:44px}h3{font-size:40px;line-height:28px}h4{font-size:36px;line-height:26px}h5{color:#444;font-size:32px;line-height:24px}h6{color:#666;font-size:28px;line-height:20px}.footer{bottom:10px;color:#ececec;left:0;right:0}.footer span{transition:all .3s}.footer span:hover{cursor:pointer}.cvbg{background-color:#ced2d6;background-position:100% 100%;background-repeat:no-repeat;padding-bottom:70px}.about{border:1px solid #9e9e9e;font-size:18px;opacity:0;overflow:hidden;padding:20px}.about p{margin-bottom:16px;opacity:0;transform:translate(-120%)}.skills{border:1px solid #9e9e9e;font-size:18px;opacity:0;overflow:hidden;padding:20px}.skills h1{font-family:\"Dongle\",var(--default-font);font-size:38px;text-transform:uppercase}.socials svg{transition:all .3s}.socials svg:hover{box-shadow:5px 5px 15px #888,-5px -5px 15px #fff;cursor:pointer}#future,#future p,#history{transform:translateY(800px)}#future p{margin-bottom:16px;opacity:0}#biography1,#biography2,#biography3,#biography4{transform:translate(-120%)}#history .divider{border-radius:var(--radius-lg);height:3px;width:100%}#history .description p{margin-bottom:16px;opacity:0;transform:translateY(800px)}#history .requirements p{--tw-scale-x:0%;--tw-scale-y:0%;--tw-scale-z:0%;background:#4630db;border-radius:var(--radius-sm);color:var(--color-indigo-50);font-size:16px;padding-block:calc(var(--spacing)*1);padding-inline:calc(var(--spacing)*2);scale:var(--tw-scale-x)var(--tw-scale-y);transform-origin:50%}#stack .tab-item{border-bottom:1px solid #bfbfbf;padding-bottom:16px;transform:translateY(1000px)}#stack .tab-item:last-child{border-bottom:0 solid #0000}#stack .tab-item span{color:var(--color-indigo-700);font-family:\"Dongle\",var(--default-font);font-size:24px;line-height:16px;text-align:right}#stack .tab-item .item-title{min-width:120px;text-align:right}#stack .progress-bar{background:#bfbfbf;border-radius:100vh;cursor:pointer;height:8px;overflow:hidden;width:min(100svw - 20px,100%)}#stack .progress-bar .progress{border-radius:inherit;height:100%;transform:scaleX(0);transform-origin:0;width:100%}#stack .progress-bar.knowledge .progress{background:linear-gradient(90deg,#4630db,#a200ff)}#stack .progress-bar.love .progress{background:linear-gradient(90deg,#df5000,#a50000)}.tabbed{overflow-x:hidden}.tabbed [type=radio]{display:none}.tabs{align-items:stretch;display:flex;gap:10px;list-style:none;margin-bottom:14px}.tab>label{border-radius:25px;color:#4630db;display:block;padding-block:calc(var(--spacing)*2);padding-inline:calc(var(--spacing)*4);transition:all .3s}.tab:hover label{background:linear-gradient(145deg,#4630db,#a200ff);box-shadow:10px 10px 30px #888,-10px -10px 30px #fff;color:#fff;cursor:pointer;transform:scale(1.1)}.tab-content{color:#141414;display:none}.tabbed [type=radio]:first-of-type:checked~.tabs .tab:first-of-type label,.tabbed [type=radio]:nth-of-type(2):checked~.tabs .tab:nth-of-type(2) label,.tabbed [type=radio]:nth-of-type(3):checked~.tabs .tab:nth-of-type(3) label{background:linear-gradient(145deg,#4630db,#a200ff);color:#fff}.tabbed [type=radio]:first-of-type:checked~.tab-content:first-of-type,.tabbed [type=radio]:nth-of-type(2):checked~.tab-content:nth-of-type(2),.tabbed [type=radio]:nth-of-type(3):checked~.tab-content:nth-of-type(3){display:flex}@property --tw-scale-x{syntax:\"*\";inherits:false;initial-value:1}@property --tw-scale-y{syntax:\"*\";inherits:false;initial-value:1}@property --tw-scale-z{syntax:\"*\";inherits:false;initial-value:1}@property --tw-translate-x{syntax:\"*\";inherits:false;initial-value:0}@property --tw-translate-y{syntax:\"*\";inherits:false;initial-value:0}@property --tw-translate-z{syntax:\"*\";inherits:false;initial-value:0}@property --tw-border-style{syntax:\"*\";inherits:false;initial-value:solid}@property --tw-gradient-position{syntax:\"*\";inherits:false}@property --tw-gradient-from{syntax:\"<color>\";inherits:false;initial-value:#0000}@property --tw-gradient-via{syntax:\"<color>\";inherits:false;initial-value:#0000}@property --tw-gradient-to{syntax:\"<color>\";inherits:false;initial-value:#0000}@property --tw-gradient-stops{syntax:\"*\";inherits:false}@property --tw-gradient-via-stops{syntax:\"*\";inherits:false}@property --tw-gradient-from-position{syntax:\"<length-percentage>\";inherits:false;initial-value:0%}@property --tw-gradient-via-position{syntax:\"<length-percentage>\";inherits:false;initial-value:50%}@property --tw-gradient-to-position{syntax:\"<length-percentage>\";inherits:false;initial-value:100%}@property --tw-shadow{syntax:\"*\";inherits:false;initial-value:0 0 #0000}@property --tw-shadow-color{syntax:\"*\";inherits:false}@property --tw-shadow-alpha{syntax:\"<percentage>\";inherits:false;initial-value:100%}@property --tw-inset-shadow{syntax:\"*\";inherits:false;initial-value:0 0 #0000}@property --tw-inset-shadow-color{syntax:\"*\";inherits:false}@property --tw-inset-shadow-alpha{syntax:\"<percentage>\";inherits:false;initial-value:100%}@property --tw-ring-color{syntax:\"*\";inherits:false}@property --tw-ring-shadow{syntax:\"*\";inherits:false;initial-value:0 0 #0000}@property --tw-inset-ring-color{syntax:\"*\";inherits:false}@property --tw-inset-ring-shadow{syntax:\"*\";inherits:false;initial-value:0 0 #0000}@property --tw-ring-inset{syntax:\"*\";inherits:false}@property --tw-ring-offset-width{syntax:\"<length>\";inherits:false;initial-value:0}@property --tw-ring-offset-color{syntax:\"*\";inherits:false;initial-value:#fff}@property --tw-ring-offset-shadow{syntax:\"*\";inherits:false;initial-value:0 0 #0000}@media (forced-colors:active){[type=checkbox]:checked,[type=radio]:checked{appearance:auto}[type=checkbox]:indeterminate{appearance:auto}[type=checkbox]:checked,[type=checkbox]:indeterminate,[type=radio]:checked{appearance:auto}}@media (min-width:96rem){.\\32 xl\\:h-\\[458px\\]{height:458px}.\\32 xl\\:h-\\[666px\\]{height:666px}.\\32 xl\\:w-\\[30\\%\\]{width:30%}.\\32 xl\\:w-\\[68\\%\\]{width:68%}.\\32 xl\\:flex-row{flex-direction:row}.\\32 xl\\:items-end{align-items:flex-end}.\\32 xl\\:justify-center{justify-content:center}.\\32 xl\\:px-10{padding-inline:calc(var(--spacing)*10)}.\\32 xl\\:pt-0{padding-top:calc(var(--spacing)*0)}.\\32 xl\\:pr-0{padding-right:calc(var(--spacing)*0)}}@media screen and (max-width:1600px){#about article,#future article,#history article,#stack .content{overflow-y:auto}:is(#stack .content,#about article,#history article,#future article)::-webkit-scrollbar{background:#a1a1a1;border-radius:6px;width:6px}:is(#stack .content,#about article,#history article,#future article)::-webkit-scrollbar-track{border-radius:6px;box-shadow:0 0 6px #a1a1a1}:is(#stack .content,#about article,#history article,#future article)::-webkit-scrollbar-thumb{background-color:#000;border-radius:6px}}@media screen and (max-width:768px){h1{font-size:38px}h2{font-size:34px}h3{font-size:30px;line-height:28px}h4{font-size:26px;line-height:26px}h5{font-size:24px;line-height:24px}h6{font-size:22px;line-height:20px}p{font-size:14px}.tab>label{border-radius:25px;color:#4630db;display:block;font-size:16px;padding-block:calc(var(--spacing)*1);padding-inline:calc(var(--spacing)*2);transition:all .3s}#history .requirements p{font-size:14px;padding-block:3px;padding-inline:6px}#stack .tab-item .item-title{min-width:90px}.tab:hover label{background:linear-gradient(145deg,#4630db,#a200ff);box-shadow:0 0 #0000,0 0 #0000;cursor:pointer;transform:scale(1)}.tab-content{padding-bottom:50px}}";
...
const AppRoot = class {
    ...
    launchCV() {
        gsapWithCSS.to(this.logo, { duration: 2, delay: 1, opacity: 1, ease: "slow" });
        gsapWithCSS.to(this.cvbg, { duration: 2, delay: 2, opacity: 1, ease: "slow" });
        gsapWithCSS.to(this.about, { duration: 2, delay: 3, y: 0, opacity: 1, ease: "expoScale" });
        gsapWithCSS.to(this.about, { duration: 2, delay: 5, borderRadius: 30, boxShadow: "20px 20px 60px #9e9e9e, -20px -20px 60px #bebebe", border: "0px solid transparent", backgroundColor: "rgba(255,255,255,0.5)", backdropFilter: "blur(4px)", ease: "expoScale", onComplete: () => this.playBiography() });
        gsapWithCSS.to(this.skills, { duration: 2, delay: 3, y: 0, opacity: 1, ease: "expoScale" });
        gsapWithCSS.to(this.skills, { duration: 2, delay: 5, borderRadius: 30, boxShadow: "20px 20px 60px #9e9e9e, -20px -20px 60px #bebebe", border: "0px solid transparent", backgroundColor: "rgba(255,255,255,0.5)", backdropFilter: "blur(4px)", ease: "expoScale" });
        gsapWithCSS.to(this.history, { duration: 1.2, delay: 6.4, y: 0, opacity: 1, ease: "expoScale", onComplete: () => this.playResponsibilities() });
        gsapWithCSS.to(this.socials, { duration: 1.4, delay: 7.4, y: 0, ease: "elastic" });
        gsapWithCSS.to(this.skillstab1, { duration: 1.4, delay: 9, y: 0, ease: "elastic" });
        gsapWithCSS.to(this.skillstab2, { duration: 1.4, delay: 9.2, y: 0, ease: "elastic" });
        gsapWithCSS.to(this.skillstab3, { duration: 1.4, delay: 9.4, y: 0, ease: "elastic" });
        gsapWithCSS.to(this.footer, { duration: 2, delay: 1, color: "#141414", ease: "slow" });
    }
    playBiography() {
    ...
    playResponsibilities() {
        ...
        descriptionitem.forEach(item => {
            gsapWithCSS.to(item, { duration: 1.2, delay: (item.attributes['data-id'].nodeValue / 4), y: 0, opacity: 1, ease: "expoScale" });
        });
        let responsibilityitem = this.el.shadowRoot.querySelectorAll('#responsibilities  p');
        responsibilityitem.forEach(item => {
            gsapWithCSS.to(item, { duration: 0.8, delay: (item.attributes['data-id'].nodeValue / 3), scale: 1, ease: "elastic.out(1,0.6)" });
        });
        gsapWithCSS.to(this.historyh3, { duration: 1.2, delay: 0.6, opacity: 1, ease: "expoScale" });
        gsapWithCSS.to(this.historyh5, { duration: 1.2, delay: 1, opacity: 1, ease: "expoScale" });
        gsapWithCSS.to(this.historyh6, { duration: 1.2, delay: 1.4, opacity: 1, ease: "expoScale" });
        gsapWithCSS.to(this.historyDivider, { duration: 1.2, delay: 1.6, x: 0, opacity: 1, ease: "expoScale" });
    }
    playStack() {
        let skillsitem = this.el.shadowRoot.querySelectorAll('#stack .tab-item');
        skillsitem.forEach(item => {
            gsapWithCSS.to(item, { duration: 1.2, delay: (item.attributes['data-id'].nodeValue / 4), y: 0, opacity: 1, stagger: 1, ease: "expoScale" });
        });
        let knowledgeprogress = this.el.shadowRoot.querySelectorAll('.knowledge .progress');
        knowledgeprogress.forEach(item => {
            gsapWithCSS.to(item, { duration: 2, delay: (item.attributes['data-id'].nodeValue / 2), scaleX: (item.attributes['data-progress'].nodeValue / 100), ease: "expoScale" });
        });
    ...
    playFuture() {
        gsapWithCSS.to(this.future, { duration: 1.2, delay: 0.5, y: 0, opacity: 1, ease: "expoScale" });
    ...
    render() {
        return (h("section", { key: 'd8fdbb7c267d6034d9c5f67604d5715928cc53d2', id: "wrapper", class: "wrapper relative w-screen min-h-screen h-fit", style: { "background-image": "url('./assets/texture.jpg')" } }, h("section", { key: 'a87f233c36c690d2256018e6339f5d702080dea5', id: "cvbg", class: "opacity-0 cvbg pt-[70px] px-5 2xl:px-10 2xl:pt-0 absolute w-screen h-screen flex flex-col 2xl:flex-row justify-end 2xl:items-end 2xl:justify-center gap-[2%]", style: { "background-image": "url('./assets/cvbg.jpg')" } }, h("section", { key: '2a967abee1bb1ddc1928cb5cc7a7f68a78e8dc0f', id: "about", class: "about w-full h-[38%] 2xl:w-[30%] 2xl:h-[666px] relative z-10" }, h("article", { key: '8a51a2411c82a09a373a0cd586e886ec4f0f7547', class: "h-full pr-3 2xl:pr-0" }, h("p", { key: '46829ecda991181f3a2aee1aac72998fb171da9b', "data-id": "1" }, "Hello there, my name is Craig du Toit and I am a Front-End Engineer and as you can clearly see, I have a passion for animation and interactivity on the web."), h("p", { key: '7ce3f3528e5bc07fd03f6e06a4da6e6b93a885f4', "data-id": "2" }, "I have been working in the industry for 15 years and this serves as not only a demonstration of my craft but also a breakdown of my recent work history, responsibilities, tech stack and hobbies."), h("p", { key: '8b9c391493ed148b476f451557c2a362a1d9909c', "data-id": "3" }, "I aspire to be a dependable and ever eager colleague, a loving husband, a good father to my son - a 2 and half year sweetheart in the hieght of the terrible twos and our newborn due in May 2026."), h("p", { key: '0e219c96143fc24d4201ff0bef4e30e8f88c547a', "data-id": "4" }, "My hobbies include running, playing padel with good friends and enjoying some seriously good food. I am a social and well-spoken person and that doesn't have a hard time making lifelong friends."), h("div", { key: '15b6c36d521a1ddacc997a0e9e1f31a3989ef5fc', id: "socials", class: "socials flex gap-2" }, h("a", { key: '4d8d68ee8f2d3772e6954061c826ebacd4c0442b', href: "https://www.linkedin.com/in/craigddutoit/", target: "_blank" }, h("svg", { key: '122c2368b3cc90ba7b9261921b31efb6db0ee183', id: "linkedin", class: "translate-y-100", xmlns: "http://www.w3.org/2000/svg", width: "36", height: "36", viewBox: "0 0 256 256" }, h("path", { key: 'bc4cb9d9204034a90ba5adcd0932e6dc9cf6da94', fill: "#480eff", d: "M218.123 218.127h-37.931v-59.403c0-14.165-.253-32.4-19.728-32.4c-19.756 0-22.779 15.434-22.779 31.369v60.43h-37.93V95.967h36.413v16.694h.51a39.91 39.91 0 0 1 35.928-19.733c38.445 0 45.533 25.288 45.533 58.186zM56.955 79.27c-12.157.002-22.014-9.852-22.016-22.009s9.851-22.014 22.008-22.016c12.157-.003 22.014 9.851 22.016 22.008A22.013 22.013 0 0 1 56.955 79.27m18.966 138.858H37.95V95.967h37.97zM237.033.018H18.89C8.58-.098.125 8.161-.001 18.471v219.053c.122 10.315 8.576 18.582 18.89 18.474h218.144c10.336.128 18.823-8.139 18.966-18.474V18.454c-.147-10.33-8.635-18.588-18.966-18.453" })))))), h("section", { key: 'fe5215915231d39baecb6dc8a3ebf311f8d5c97d', id: "skills", class: "skills w-full h-[60%] 2xl:w-[68%] 2xl:h-[666px] tabbed relative z-10" }, h("input", { key: '963b5743f33bcca2bc21cd975524ec22a143344d', type: "radio", id: "tab1", name: "css-tabs", checked: true }), h("input", { key: 'f7a78da9f59e45b347db233d56daa343410c8199', type: "radio", id: "tab2", name: "css-tabs" }), h("input", { key: 'd0c1226781e50878344eae9b0396aa2ca38350f6', type: "radio", id: "tab3", name: "css-tabs" }), h("ul", { key: '592011c43e3387b5252baaf5432f7c4b51dabd3d', class: "tabs" }, h("li", { key: '5734c47790be26cdfcfb40338e116f86af3eac84', id: "skillstab1", class: "tab -translate-y-28" }, h("label", { key: '26ad58b9cff32b1ab0ef50193dc33f6c5b97bf93', htmlFor: "tab1" }, "Position Held")), h("li", { key: '9d4dfd256f202d04aa142aea819cb20984ee9fcd', id: "skillstab2", class: "tab -translate-y-28", onClick: () => this.playStack() }, h("label", { key: '66f13095dfce954d58f7fa09d3ca0bacf9775e98', htmlFor: "tab2" }, "Stack")), h("li", { key: 'c333c695795e3a590c4d548709fbe3773986c258', id: "skillstab3", class: "tab -translate-y-28", onClick: () => this.playFuture() }, h("label", { key: '005aae6d3163a6cf28536283ff3696563965d30c', htmlFor: "tab3" }, "The Future"))), h("main", { key: '71905019dbfadcb8d47328540bab3f86475a698a', id: "history", class: "tab-content h-full" }, h("article", { key: '1edd2898492ad6bc15863ac2a2adc25663c8566b', class: "tab-item h-full 2xl:h-[458px] pr-3 2xl:pr-0" }, h("h3", { key: '703f16c0bca50cff68ebb083b7cf41845d011dfd', class: "opacity-0" }, "Software Developer - Team Lead | Promotions & Retention Marketing"), h("h5", { key: '2840926dad02575c6eff064db79c8327309c0e06', class: "opacity-0" }, "Osiris Trading \u00B7 Full-time"), h("h6", { key: '50665687b9515c1875b0c2de2538ba4c144d864f', class: "opacity-0" }, "Nov 2021 - Present \u00B7 ", this.dateHandler('2021-11-01T10:00:00Z'), " "), h("div", { key: 'dac1c52c5e41cda45edf779a4f42ec91d37d2a93', class: "mt-4 mb-3 divider bg-gradient-to-r from-indigo-700 via-40% via-transparent to-transparent -translate-[120%]" }), h("div", { key: '431f34f46bcfc4e7f9e12e34c95376912eebc0e8', class: "flex flex-col h-full justify-between" }, h("div", { key: '3a4ba9a8abcbf7a14af47ee7242f32c1b610363b', id: "description", class: "description" }, h("p", { key: 'f1122ade3e2d08d06f04162399bb4b972f3f078b', "data-id": "1" }, "Building bespoke promotions focused on delivering engaging experiences for the global Betway Brand. Utilizing the latest technology to deliver fast, data efficient and visually stunning UI and UX projects. Working with a team of dedicated back-end and front-end engineers catering to both business and customer needs."), h("p", { key: 'd42f2cb79234bad42e83ad8a07cdd83c5c2cde97', "data-id": "2" }, "I am in charge of project planning, understanding business requirements and delivering above expected quality on project delivery whilst standardizing team's understanding to deliver similar results."), h("p", { key: '6318779ea7be819fc4d993d253915616f4d184ef', "data-id": "3" }, "Feel free to visit my ", h("a", { key: '91ed1a66986d3c79c569d4eb261379093f29081c', class: "text-indigo-700", href: "https://www.linkedin.com/in/craigddutoit/", target: "_blank" }, "LinkedIn Profile"), " for broader career experience.")), h("div", { key: '69eeb44652c1ee28897f44a8a639f19e24532d2f', class: "requirements" }, h("h5", { key: '410e920e1e68a0c531c7b1010860649e0f4cc982' }, "Responsibilities:"), h("div", { key: '2d1ba2f53b816404995438c58cf9c7efaf23d138', id: "responsibilities", class: "flex gap-1 flex-wrap" }, h("p", { key: '88634c03597702a214602472686a54d27a1661fe', "data-id": "1" }, "Defining design"), h("p", { key: 'd4fbb9735e3bfe86f6f210d0521e6821806e0552', "data-id": "2" }, "Developing Front-End"), h("p", { key: 'd88156899c2b000b1143ef70b96aa4d42a1edc67', "data-id": "3" }, "Collaboration on Design Refinement"), h("p", { key: 'd2658ddc7970bdcc197ba84d591def45aeb31cf2', "data-id": "4" }, "Collaboration on Back-end Data structures"), h("p", { key: '4e3a67b48e5fabf5b462b96a871f59ada2f7f0e6', "data-id": "5" }, "Project Planning"), h("p", { key: 'a5fabf5029a678b877ea36533938773eb9262256', "data-id": "6" }, "Team Planning"), h("p", { key: 'ad7d8372c95bd1097a45d323d8b2b59faae98695', "data-id": "7" }, "Project Feedback sessions"), h("p", { key: 'c1e53fa44d47e71ebb0c0617f8a539abff0d4c2f', "data-id": "8" }, "Ongoing support of live software"), h("p", { key: '82bff5ca6975f9acf0d752828ed601454abe54b1', "data-id": "9" }, "Future Proofing Project Structures"), h("p", { key: '26f7ba66f9d0e0abc5b7fc6a11f38bd758e48c6a', "data-id": "10" }, "Creation & Maintenance of Front-End Brand UI"), h("p", { key: 'a5988dd35fdf49b3e831565c5e98098db4ced26b', "data-id": "11" }, "Mentorship & Training")))))), h("main", { key: '8315078d4f152afd5e5e97c35247d1e1689e85f1', id: "stack", class: "tab-content h-full 2xl:h-[458px]" }, h("div", { key: '5e89ef573d960ec485c889854c8090293721d5a9', class: "content w-full h-full flex flex-col gap-4 pr-3 2xl:pr-0" }, h("article", { key: 'c04936a650a122bab23bb9914c676ff73db598be', "data-id": "1", class: "tab-item w-full flex gap-6 items-center" }, h("div", { key: 'cec358ee767b87893cfab1d76ce5cd37b8d2547f', class: "item-title flex flex-col" }, h("h5", { key: 'a1d08e53ff9553e4cf616ebcdfe5ad006417926d' }, "StencilJS"), " ", h("span", { key: 'ad33537494ec29fe2b9481d3fbc9736dd2cb4ff7' }, "4 YEARS")), h("div", { key: 'a59a8357281c7b023aef01ee41823c8e510c28c0', class: "flex w-full flex-col gap-2" }, h("h6", { key: '30be680b05e48044fe437e842b33d8417f952653' }, "Knowledge"), h("div", { key: '3187245debd760eaa245cfebdb818b8a0afbb92b', class: "progress-bar knowledge" }, h("div", { key: '6640376be6814dc946a91d2c91a23738dc8099d6', "data-id": "1", "data-progress": "90", class: "progress" }))), h("div", { key: 'df4d542d0efd6a6626261ae3e4a127fca5b3b46e', class: "flex w-full flex-col gap-2" }, h("h6", { key: '73062bbd6413ac195ce14d38271578d3aa351f80' }, "Love"), h("div", { key: 'f85d91f07cd5913235e838b9e826f4492c8b5bd3', class: "progress-bar love" }, h("div", { key: '716498aa9de8c6b58dc66b8ee4b1ee3ed8248248', "data-id": "1", "data-progress": "100", class: "progress" })))), h("article", { key: '2468ccbd8e695ff0d05827738ee4e37cdd197ef3', "data-id": "2", class: "tab-item w-full flex gap-6 items-center" }, h("div", { key: 'd3d0d6d080de6e588eb15c1e87566c62f2b1738f', class: "item-title flex flex-col" }, h("h5", { key: '86d00b2608e1ff415b38f8b4c40bc30a95806097' }, "React"), " ", h("span", { key: '498f2d1ce03623513b87a67ec13489c485f8aeaa' }, "2 YEARS")), h("div", { key: 'c37a9411b4aee7b8dc9cc1f41dec32041fe518c7', class: "flex w-full flex-col gap-2" }, h("h6", { key: '75112a01820a8b043fe9eba1e9e36452e6066612' }, "Knowledge"), h("div", { key: '91aa22ce306df938367f7d5dab70a33adb52590d', class: "progress-bar knowledge" }, h("div", { key: 'aa20fe0d9c75252f0d657e14407eebaca1ee1ba7', "data-id": "2", "data-progress": "70", class: "progress" }))), h("div", { key: '6e75ee79165ddaf8612c1c087549b063507b6195', class: "flex w-full flex-col gap-2" }, h("h6", { key: '41814785bec7c55dd4dbe3ddde086c1d1bb92013' }, "Love"), h("div", { key: '9a668a92cfcb1ba8f2dbb16b81b5de3fe3e18fcf', class: "progress-bar love" }, h("div", { key: 'b32a90ca8fea9fd6fd2fda36bbc24c7c796d2f13', "data-id": "2", "data-progress": "90", class: "progress" })))), h("article", { key: '2307a9b4f3e00e7bd3c8291775c7a202d56bf8f7', "data-id": "3", class: "tab-item w-full flex gap-6 items-center" }, h("div", { key: '202a653eb85f8a1db2cd47f11ff6b1ea94e22573', class: "item-title flex flex-col" }, h("h5", { key: 'c0bf7b43a4971199f289157994a218dae28872f1' }, "Angular"), " ", h("span", { key: 'cd97652f70224c089aad650ea48fe422af10d96a' }, "2 YEARS")), h("div", { key: '040506340534fb33c8b622e46efe739632e2a223', class: "flex w-full flex-col gap-2" }, h("h6", { key: '55ce6bf497da4a08b794cd96f266ae0ed7fa3826' }, "Knowledge"), h("div", { key: '2688eb5f0f54b10c6b305a0620a24185ff58b16f', class: "progress-bar knowledge" }, h("div", { key: 'd094185f5074b296a302a601fba9aa83fa651e6f', "data-id": "3", "data-progress": "70", class: "progress" }))), h("div", { key: 'ab7705c72a60b8565717105cc9c93726d57fe809', class: "flex w-full flex-col gap-2" }, h("h6", { key: '3b7fc62384d0e5c170353316e9bc0f2a2014cec9' }, "Love"), h("div", { key: '6480419a7a9d7cc578371efb0ac9ce29c1a4856b', class: "progress-bar love" }, h("div", { key: '77cd59d6991edfe224e35f7a62ab844051ebeb78', "data-id": "3", "data-progress": "70", class: "progress" })))), h("article", { key: '272448d55f5f4caec7f5e9f952e72f444bee95e4', "data-id": "4", class: "tab-item w-full flex gap-6 items-center" }, h("div", { key: '342383bbd82c37c446ada5e678ff35c2c3e2a8dd', class: "item-title flex flex-col" }, h("h5", { key: '839fcb44a852e728cfea45577b9ce3c7087227c2' }, "TypeScript"), " ", h("span", { key: '9b669ae2361b39bfb39fe4a7e5a949422575065c' }, "5 YEARS")), h("div", { key: '3fb69ea16d112eb623049fc5244f53982290d6a9', class: "flex w-full flex-col gap-2" }, h("h6", { key: '7c272248dc7a80d3439c668025c38e06965a2077' }, "Knowledge"), h("div", { key: 'b0f9e6e99cdb885d5186d77672d80645098d2370', class: "progress-bar knowledge" }, h("div", { key: 'a7c87babc9006d25f1226c9ee35780ed2433a2a1', "data-id": "4", "data-progress": "80", class: "progress" }))), h("div", { key: '1bf43d5186e1924b6f9cd04a3443f13d1e7d2156', class: "flex w-full flex-col gap-2" }, h("h6", { key: 'd37eb218ed617c401a4323c71fb8885fda308429' }, "Love"), h("div", { key: '2402dd6fa1e1925e536e42243effc383a317c303', class: "progress-bar love" }, h("div", { key: 'c153d65e63c6589fe6cc2237a021eca4d638dd52', "data-id": "4", "data-progress": "90", class: "progress" })))), h("article", { key: '34b564c33f42181d6b8edb80c048628f76dba9bd', "data-id": "5", class: "tab-item w-full flex gap-6 items-center" }, h("div", { key: '6893ae12dab6de1d6ab52aa66840abfafb7cb02b', class: "item-title flex flex-col" }, h("h5", { key: 'af1c909702c6ca9920a1109b9f11e6b586818453' }, "GSAP"), " ", h("span", { key: '1624c9b25e3efdfa37091070a0dbb0555c3eee43' }, "2 YEARS")), h("div", { key: '2594f74cc28739bd45c7e7e640768d0c23a4e014', class: "flex w-full flex-col gap-2" }, h("h6", { key: 'dfabe93e2dd41376b03177cabfe012b2584acb25' }, "Knowledge"), h("div", { key: '3bfd2b889af9cf41b00a426169e61b06ba23edd5', class: "progress-bar knowledge" }, h("div", { key: '6ef107b6b8d7f5c4f98aec42ceaf007cf34b68bf', "data-id": "5", "data-progress": "70", class: "progress" }))), h("div", { key: 'c0d7c6aeeca8fa9709516b02756140224963e969', class: "flex w-full flex-col gap-2" }, h("h6", { key: '37319ae36e535ec60397aca9d8e602b1645e5f44' }, "Love"), h("div", { key: 'ba77ca47d22a7340ef75dcb1d089f72f8c257070', class: "progress-bar love" }, h("div", { key: 'db97c140602777ef16b7cbc64c57b22a65858408', "data-id": "5", "data-progress": "100", class: "progress" })))), h("article", { key: 'e6c67997ff11b0a16376fc7af80c3795a884baeb', "data-id": "6", class: "tab-item w-full flex gap-6 items-center" }, h("div", { key: 'b9a10dc5425b79ea7e2e6d956d06547525e6efc5', class: "item-title flex flex-col" }, h("h5", { key: 'e7081375a2dab7522a0d9dde003c6dc96f9ea4b5' }, "TailwindCSS"), " ", h("span", { key: '303d8c2739b8f8debfaf74e4fcf7ff19b4edb57e' }, "3 YEARS")), h("div", { key: 'f75d4e3066d30b13e4f1e7d1c698cfcbecc422b3', class: "flex w-full flex-col gap-2" }, h("h6", { key: '7c72acb3c25499095aa7bbbdee6b896a1f7c306a' }, "Knowledge"), h("div", { key: '3cee7540113a38af6ecd3ad0b2affed4f19ce567', class: "progress-bar knowledge" }, h("div", { key: '76d930bc18712c569c53c82d8001fb282e833069', "data-id": "6", "data-progress": "90", class: "progress" }))), h("div", { key: 'e45c504d90ad8a01e93644c281849a44b346f5e5', class: "flex w-full flex-col gap-2" }, h("h6", { key: 'f6d5b3fa51fe5343866fd08dedd0467d8f9edf8c' }, "Love"), h("div", { key: '1163e93f59bfcac8055fd64154a4705da0ab1cf3', class: "progress-bar love" }, h("div", { key: '09c3fa074ffc8d651b32c042edc03bbb0d9ebf70', "data-id": "6", "data-progress": "90", class: "progress" })))), h("article", { key: '61605638edf201bc89caa489006ffa666eb5e4db', "data-id": "7", class: "tab-item w-full flex gap-6 items-center" }, h("div", { key: 'c9942689b16b50a12c37a2bb3f08e82a38cb1045', class: "item-title flex flex-col" }, h("h5", { key: 'f7521ab5b024b3664341b30244d2a4a586bf5bbf' }, "CSS"), " ", h("span", { key: 'c9609833fe4cc23ddf7ef4ebf51edad3b87e759a' }, "15 YEARS")), h("div", { key: '5681b831c9c293d442a2b7db82db89baba410bc3', class: "flex w-full flex-col gap-2" }, h("h6", { key: '4b01e7aeb27f4742229fefea8a048f056e83a85a' }, "Knowledge"), h("div", { key: 'de80db939041b2b07ae814e3ae51b84a035c17ca', class: "progress-bar knowledge" }, h("div", { key: 'a2d46f2d62d3f0c3f6acf374b422c9757033b89a', "data-id": "7", "data-progress": "100", class: "progress" }))), h("div", { key: '2221e5e4afc17d9a2dde5bd68040e46f28fb7dd9', class: "flex w-full flex-col gap-2" }, h("h6", { key: '049f5860f122c929923d6d3f8091d4dab5b370c6' }, "Love"), h("div", { key: '46e1c4097b34d991fa264798a1447134ae863068', class: "progress-bar love" }, h("div", { key: '9d5e593afd83fabc785d9d28239e2fe35310444f', "data-id": "7", "data-progress": "100", class: "progress" })))), h("article", { key: 'db3491f58efdf3098a8b0e89f868fb7265c6597f', "data-id": "8", class: "tab-item w-full flex gap-6 items-center" }, h("div", { key: '29aa5a3dfbfcf8d4d55d613901cc43532128a640', class: "item-title flex flex-col" }, h("h5", { key: '54413020e0773e280f7735d8ad2bb0265abb30f8' }, "Git"), " ", h("span", { key: '10897617f99f85964381f6c423b3761d683656f5' }, "15 YEARS")), h("div", { key: '2e4eecf45a33b9277477eec39e7f935fb4b20ab1', class: "flex w-full flex-col gap-2" }, h("h6", { key: '75f4d19b85984372b983af05149b275fec565dad' }, "Knowledge"), h("div", { key: 'eb6a34eb01260f33d56bc1ad1560b9f20cf3d1a1', class: "progress-bar knowledge" }, h("div", { key: 'b4fb84c7e42183033f217ee21e905b09bf1d95a5', "data-id": "8", "data-progress": "80", class: "progress" }))), h("div", { key: '8f03e3eaf5e27d6f2215a7f5699633116efcdef5', class: "flex w-full flex-col gap-2" }, h("h6", { key: 'd0e4e8879ab88a92f16706755623e91b8d952490' }, "Love"), h("div", { key: '1de2172c0e10770efe9a158f48b680d2dfb3a6a9', class: "progress-bar love" }, h("div", { key: 'd832472347f0e31f93f6f7622444a9395db8a5b0', "data-id": "8", "data-progress": "100", class: "progress" })))))), h("main", { key: 'fb53af3657806a26db9851462e53774978e69acd', id: "future", class: "tab-content h-full" }, h("article", { key: 'b5123fc7930c40255a0335081f28e9d191565aa6', class: "tab-item h-full pr-3 2xl:pr-0" }, h("p", { key: 'f09b7ccb4d439d78648137656aabcc71f1d7fe62', "data-id": "1" }, "I would not neccessarily say I am open to all offers when it comes to employment, I simultaneuosly enjoy what I do and where I am doing it which seems to be a rare thing these days, and therefore I have that wonderful luxury in being able to choose comfortably what comes next."), h("p", { key: 'a23dbabf8bea02144e2dc9e2f6d7861b01fbad80', "data-id": "2" }, "With that being said, my mind does wander to what may lie for me out there, to what challenges await, perhaps an exciting endeavor, perhaps amazing people with enough passion about what they do that one can hardly pass up the offer."), h("p", { key: 'cda2d96b93681d8bf577e6bbb7145f7d41f455e3', "data-id": "3" }, "I believe in brands and business built on the love of the game, numbers are a conversation of course, but that passion for what you do and who you do it with... that makes one dream of waking up to the work ahead."), h("p", { key: 'df30ee93bfb5bf3dcecb6a49be92c53ec786a2ed', "data-id": "4" }, "Please feel free to contact me to discuss your passion and vision."), h("p", { key: 'de771b9eb0e162a111bb90712fadee692d71b24e', "data-id": "5" }, "Mail me ", h("a", { key: 'b802d2d0970e6e81c91452459c2ee92baf7014ed', href: "mailto: craigtoit@gmail.com", class: "text-indigo-700" }, "craigtoit@gmail.com"), "."))))), h("div", { key: 'd3b6363c31c98868048ae40f8df789b1d796566c', id: "logo", class: "logo opacity-0 absolute w-full z-0" }, h("img", { key: '62439cf50f7b9472e2c98b6f3abb7d7e911dd217', class: "absolute -left-[278px]", src: "./assets/logo.png", alt: "" })), h("p", { key: 'a97ef9a371189e2dd7801ef6ac17191a8ae5dd36', id: "footer", class: "footer absolute text-lg mx-auto w-fit h-fit font-sans" }, "Hand built with StencilJS + GSAP + Tailwind CSS")));
    }
    componentDidLoad() {
